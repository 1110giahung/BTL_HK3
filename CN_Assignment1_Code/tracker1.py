...
class server:
    ...
    def update_file_from_one_peer(self, files, client_socket):          # files thuoc server
                                                                        # client_info thuoc client
        with self.lock:
            for file in files:
                if file not in self.files:                                          # xem tren server co ko
                    self.files[file] = files[file][1]

                if file not in self.client_info[client_socket]:                     # xem tren may co file ko thi update ve
                    self.client_info[client_socket][file] = files[file][0]
                else:
                    for index in files[file][0]:                                    # neu co file nay thi update lai
    ...
    def find_peer_have(self, target_file):
        peer_list = []      # tao array chua ng co file
    ...
    def handle_specific_client(self, client_socket : socket.socket, client_addr):
        ...
        while self.running: 
            ...
            try:
                message = self.parse_message(message) #tach ;
            except Exception as e:
            ...
            if message["type"] == mess_type.HANDSHAKE.value:
                ...
                peer_list = self.find_peer_have(target_file)            #array
                message["file"][target_file] = self.files[target_file]     # cap nhat message cua client giong server
                ...
                with self.lock:
                    if client_socket in self.client_info:
                        for file in self.client_info[client_socket]:    # file cua 1 nguoi
                            flag = False
                            for client in self.client_info:     # nguoi do thuoc dsach      client_socket=3    
                                if client == client_socket:                 #   1 2 3 4 5
                                    continue                                #   a b c d e
                                if file in self.client_info[client]:        #   l k j
                                    flag = True                             #   o p i        
                                    break
                            if not flag:
                                del self.files[file]
